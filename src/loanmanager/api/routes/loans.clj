(ns loanmanager.api.routes.loans
  (:require [next.jdbc :as jdbc]
            ...
            [loanmanager.domain.workflow :as workflow]
  ...
  [;; ── Applications ──────────────────────────────────────────────────────────
                                ...
                                score-result (credit-score/score
                                               {:employment-years         (or (:customers/employment-years customer) 0)
                                                ...
                                                :dti                      (finance/debt-to-income
                                                                            ...
                                                                            obligs
                                                                            (finance/reducing-balance-payment
                                                                              requested-amount
                                                                              (/ (:loan-products/interest-rate-min product) 100)
                                                                              requested-duration))
                                                :active-facilities        1
                          ...
                          (let [app-id  (parse-uuid (:id path-params))
                                ...
                                loan    (loans-db/create-loan! ds
                                          {:tenant-id             tenant-id
                                           ...
                                           :repayment-freq        (name (:loan-products/repayment-frequency product))
                                           :currency              (:loan-products/currency product)
     ...
     :post {:summary    "Record a payment"
            ...
            :handler    (fn [{:keys [identity tenant-id path-params body-params]}]
                          ...
                          (let [loan-id (parse-uuid (:id path-params))
                                ...
                                interest-due (* (:loans/outstanding-principal loan)
                                                (/ rate 12))
                            ...
                            (loans-db/update-loan! ds tenant-id loan-id
                              {:outstanding-principal (- (:loans/outstanding-principal loan)
                               ...
                               :total-paid-principal  (+ (:loans/total-paid-principal loan)
                                                         principal-portion)
                               :total-paid-interest   (+ (:loans/total-paid-interest loan)
                                                         interest-portion)
   ...
   ["/loans/simulate"
    {:post {:summary    "Simulate loan repayment schedule (no auth required)"
            ...
            :handler    (fn [{{:keys [principal annual-rate months]} :body-params}]
                          (let [schedule (finance/build-schedule
                                           {:principal   principal
                                            :annual-rate (/ annual-rate 100)
                                            :months      months})
                                total-interest (finance/total-cost schedule)]
                            {:status 200
